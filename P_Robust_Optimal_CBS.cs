...
namespace CPF_experiment
{
    ...
    /// </summary>
    public class P_Robust_Optimal_CBS_LocalConflicts : ICbsSolver
    {
        /// <summary>
        ...
        /// </summary>
        public Dictionary<P_Robust_Optimal_CbsNode, P_Robust_Optimal_CbsNode> closedList;
        ...
        public int highLevelExpands;
        public int totalCost;
        protected int solutionDepth;
        public Run runner;
        protected P_Robust_Optimal_CbsNode goalNode;
        ...
        public double probust;
        public double delay_prob;
        public bool pRobust_Empirically;
        ...
        public P_Robust_Optimal_CBS_LocalConflicts(ICbsSolver singleAgentSolver, ICbsSolver generalSolver, int mergeThreshold = -1,
                                  ...
                                  int lookaheadMaxExpansions = int.MaxValue, bool mergeCausesRestart = false, int conflictRange = 0, Run.ConstraintPolicy constraintPolicy = Run.ConstraintPolicy.Single, double probust_prob = 0, double delay_prob = 0, bool pRobust_Empirically = false)
        {
            this.closedList = new Dictionary<P_Robust_Optimal_CbsNode, P_Robust_Optimal_CbsNode>();
            this.openList = new OpenList(this);
            this.mergeThreshold = mergeThreshold;
            this.conflictRange = conflictRange;
            //this.rangeConstraint = rangeConstraint;
            this.constraintPolicy = constraintPolicy;
            this.probust = probust_prob;
            this.delay_prob = delay_prob;
            this.pRobust_Empirically = pRobust_Empirically;
        ...
        {
            TimedMove.conflictProbability = this.delay_prob;
            ...
            P_Robust_Optimal_CbsNode root = new P_Robust_Optimal_CbsNode(instance.m_vAgents.Length, this.solver, this.singleAgentSolver, this); // Problem instance and various strategy data is all passed under 'this'.
            // Solve the root node
        ...
        {
            this.highLevelExpands = highLevelExpanded;
        ...
        protected bool SetGlobals()
        {
            ...
            if (this.instance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.CAT) == false) // Top-most CBS solver
            {
                this.instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT] = new Dictionary_U<TimedMove, int>(); // Dictionary_U values are actually lists of ints.
                this.instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS] = new HashSet_U<P_Robust_Optimal_CbsConstraint>();
                //if (this.doMalte && this.instance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS) == false)
                    this.instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS] = new HashSet_U<P_Robust_Optimal_CbsConstraint>();
                return true;
            }
            else
                return false;
        ...
        protected void CleanGlobals()
        {
            ...
            {
                this.instance.parameters.Remove(P_Robust_Optimal_CBS_LocalConflicts.CAT);
                this.instance.parameters.Remove(P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS);
        ...
        public bool Solve()
        {
            ...
            if (openList.Count > 0)
                initialEstimate = ((P_Robust_Optimal_CbsNode)openList.Peek()).totalCost;
            ...
            while (openList.Count > 0)
            {
                ...
                if (runner.ElapsedMilliseconds() > Constants.MAX_TIME)
                {
                    ...
                    this.solutionDepth = ((P_Robust_Optimal_CbsNode)openList.Peek()).totalCost - initialEstimate; // A minimum estimate
                    this.Clear(); // Total search time exceeded - we're not going to resume this search.
                    this.CleanGlobals();
                    return false;
                }

                var currentNode = (P_Robust_Optimal_CbsNode)openList.Remove();
                if (currentNode.allSingleAgentPlans == null)
                    continue;
                ...
                if (this.openList.Count != 0 &&
                    currentNode.f > ((P_Robust_Optimal_CbsNode)this.openList.Peek()).f &&
                ...
                this.addToGlobalConflictCount(currentNode.GetConflict()); // TODO: Make P_Robust_Optimal_CBS_GlobalConflicts use nodes that do this automatically after choosing a conflict
                ...
                if (this.useMddH)
                {
                    if (currentNode.h == 0 && // Otherwise the node's h was already computed. It may have been computed if h is zero, but then the lookup will find the h quickly. This is just a minor optimization.
                        ...
                        currentNode.f == ((P_Robust_Optimal_CbsNode)this.openList.Peek()).f) // We can only add 1 to the h so a push back can only happen if the F of the next node is equal to the current one's before starting.
                    // TODO: Generalize DynamicLazyOpenList to fit CBS instead of this logic duplication
                    {
                        ...
                        else
                        {
                            ...
                            currentNode.h = 1;
                            var next = (P_Robust_Optimal_CbsNode)this.openList.Peek();
                ...
                // Expand
                bool wasUnexpandedNode = (currentNode.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED &&
                                         currentNode.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED);
                if (currentNode.conflict != null)
                Expand(currentNode);
                if (wasUnexpandedNode)
                    highLevelExpanded++;
                // Consider moving the following into Expand()
                if (currentNode.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED &&
                    currentNode.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED) // Fully expanded
                    currentNode.Clear();
            }
        ...
        protected virtual bool ShouldMerge(P_Robust_Optimal_CbsNode node)
        {
        ...
        protected void Shuffle(P_Robust_Optimal_CbsNode currentNode)
        {
            if (currentNode.agentAExpansion != P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED || currentNode.agentBExpansion != P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED)
            ...
            while (currentNode.GoalTest() == false)
            {
                ...
                if (
                    ...
                    ( // Check that the first agent in the conflict wasn't just replanned or merged - shuffling in that case would necessarily have the same result since the low level is Optimal
                     conflictChanged == false &&
                     currentNode.prev != null &&
                     (
        ...
        protected bool ExpandImpl(P_Robust_Optimal_CbsNode node, bool adopt, out IList<P_Robust_Optimal_CbsNode> children, out bool reinsertParent, P_Robust_Optimal_CbsNode adoptBy = null)
        {
            CbsConflict conflict = node.GetConflict();
            children = new List<P_Robust_Optimal_CbsNode>();
            if (adoptBy == null)
                adoptBy = node;
            ushort adoptByH = adoptBy.h;

            P_Robust_Optimal_CbsNode child;
            ...
            if (this.mergeThreshold != -1 && ShouldMerge(node))
            {
                ...
                else
                {
                    ...
                    child = new P_Robust_Optimal_CbsNode(this.instance.m_vAgents.Length, this.solver, this.singleAgentSolver, this, node.agentsGroupAssignment);
            ...
            // Generate third child for optimal p-robust:
            if (node.conflict.timeStepAgentA != 0  && node.conflict.timeStepAgentB != 0)
            {
                child = ConstraintExpand(node, false, out closedListHitChildCost, true);
                if (child != null)
                {
                    if (child == node) // Expansion deferred
                        reinsertParent = true;
            ...
            if (node.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED || node.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED)
        ...
        public virtual void Expand(P_Robust_Optimal_CbsNode node)
        {
            ushort parentCost = node.totalCost;
            ushort parentH = node.h;
            IList<P_Robust_Optimal_CbsNode> children = null; // To quiet the compiler
            ...
            if (this.conflictChoice == ConflictChoice.FIRST || this.conflictChoice == ConflictChoice.MOST_CONFLICTING) // They just a conflict once and stick with it.
            {
                ...
                else if (this.bypassStrategy == BypassStrategy.FIRST_FIT_LOOKAHEAD || node.parentAlreadyLookedAheadOf) // Do bypass but don't look ahead
                {
                    ...
                    while (true) // Until a node with a higher cost is found or a goal is found
                    {
                        ...
                        var lookAheadSameCostNodes = new HashSet<P_Robust_Optimal_CbsNode>();
                        var lookAheadLargerCostNodes = new HashSet<P_Robust_Optimal_CbsNode>();
                        var lookAheadSameCostNodesToReinsertWithHigherCost = new HashSet<P_Robust_Optimal_CbsNode>();
                        ...
                        IList<P_Robust_Optimal_CbsNode> lookAheadChildren;
                        ...
                        while (lookAheadOpenList.Count != 0)
                        {
                            ...
                            P_Robust_Optimal_CbsNode lookAheadNode = (P_Robust_Optimal_CbsNode)lookAheadOpenList.Remove();
                        ...
                        if (adopted == false) // Insert children into open list
                        {
                            ...
                            children = lookAheadLargerCostNodes.ToList<P_Robust_Optimal_CbsNode>(); // Larger cost nodes aren't expanded so they're always on the frontier
                            int unexpandedSameCostNodes = lookAheadOpenList.Count;
                            while (lookAheadOpenList.Count != 0)
                            {
                                var child = (P_Robust_Optimal_CbsNode)lookAheadOpenList.Remove();
                            ...
                            foreach (P_Robust_Optimal_CbsNode lookAheadNode in lookAheadLargerCostNodes)
                        ...
                        else // Adopted. Do a new round of expansions.
                        {
                            foreach (P_Robust_Optimal_CbsNode lookAheadNode in lookAheadLargerCostNodes)
                                this.closedList.Remove(lookAheadNode); // Just so they'll be inserted into the open list at the end of the method
                            foreach (P_Robust_Optimal_CbsNode lookAheadNode in lookAheadSameCostNodes)
                                this.closedList.Remove(lookAheadNode); // Just so they'll be inserted into the open list at the end of the method
                ...
                else // this.bypassStrategy == BypassStrategy.BEST_FIT_LOOKAHEAD and this set of costs not already done
                {
                    ...
                    {
                        IList<P_Robust_Optimal_CbsNode> lookAheadSameCostNodesSerialzed = lookAheadSameCostNodes.ToList<P_Robust_Optimal_CbsNode>();
                        P_Robust_Optimal_CbsNode adoptionCandidate = lookAheadSameCostNodesSerialzed[0];
                        ...
                        if (AdoptConditionally(node, adoptionCandidate, parentH))
                        {
                            ...
                            if (this.openList.Count != 0 && ((P_Robust_Optimal_CbsNode)this.openList.Peek()).f < node.f)
                            ...
                            {
                                reinsertParent = true;
                                children = new List<P_Robust_Optimal_CbsNode>(); // Children will be generated when this node comes out of the open list
                    ...
                    if (bypassPerformed == false)
                    {
                        ...
                        children = lookAheadLargerCostNodes.ToList<P_Robust_Optimal_CbsNode>();
                        int unexpandedSameCostNodes = lookAheadOpenList.Count;
                        while (lookAheadOpenList.Count != 0)
                        {
                            var child = (P_Robust_Optimal_CbsNode)lookAheadOpenList.Remove();
            ...
            else // We try to split according to cardinal conflicts
            {
                ...
                while (true)
                {
                    ...
                    if (
                        ...
                        ((node.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED && node.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED) || // Both children generated by now
                        (node.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED && node.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED) || // One child still deferred, will surely have a higher cost (that's why it was deferred)
                        (node.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED && node.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED)) // One child still deferred, will surely have a higher cost (that's why it was deferred)
                            ) // A cardinal conflict
                    {
                        if (this.debug)
                    ...
                    node.agentAExpansion = P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED;
                    node.agentBExpansion = P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED;
                    node.h = parentH;
                    // Take care of counts:
        ...
        protected bool MDDHeuristic(P_Robust_Optimal_CbsNode node)
        {
        ...
        protected P_Robust_Optimal_CbsNode MergeExpand(P_Robust_Optimal_CbsNode node, out int closedListHitChildCost)
        {
            CbsConflict conflict = node.GetConflict();
            closedListHitChildCost = -1;
            
            P_Robust_Optimal_CbsNode child = new P_Robust_Optimal_CbsNode(node, node.agentsGroupAssignment[conflict.agentAIndex], node.agentsGroupAssignment[conflict.agentBIndex]);
        ...
        protected P_Robust_Optimal_CbsNode ConstraintExpand(P_Robust_Optimal_CbsNode node, bool doLeftChild, out int closedListHitChildCost, bool doThirdChild = false)
        {
            CbsConflict conflict = node.GetConflict();
            int conflictingAgentIndex = doLeftChild? conflict.agentAIndex : conflict.agentBIndex;
            int conflictingAgent2Index = (!doLeftChild) ? conflict.agentAIndex : conflict.agentBIndex;
            if (!doThirdChild)
                conflictingAgent2Index = -1;
            //CbsNode.ExpansionState expansionsState = doLeftChild ? node.agentAExpansion : node.agentBExpansion;
            P_Robust_Optimal_CbsNode.ExpansionState expansionsState = P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED;
            P_Robust_Optimal_CbsNode.ExpansionState otherChildExpansionsState = doLeftChild ? node.agentBExpansion : node.agentAExpansion;
            ...
            if ((Constants.Variant == Constants.ProblemVariant.ORIG &&
                expansionsState == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED && conflict.vertex == true &&
            ...
            {
                if (otherChildExpansionsState == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED && conflictRange != -1)
                ...
                if (doLeftChild)
                    node.agentAExpansion = P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED;
	            else
                    node.agentBExpansion = P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED;
            ...
            else if (expansionsState != P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED)
            // Agent expansion already skipped in the past or not forcing it from its goal - finally generate the child:
            {
                ...
                if (doLeftChild)
                    node.agentAExpansion = P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED;
                else
                    node.agentBExpansion = P_Robust_Optimal_CbsNode.ExpansionState.EXPANDED;

                P_Robust_Optimal_CbsConstraint newConstraint;
                P_Robust_Optimal_CbsNode child;
                if (!doThirdChild)
                {
                    newConstraint = new P_Robust_Optimal_CbsConstraint(conflict, instance, doLeftChild, constraintPolicy, conflictRange);
                    child = new P_Robust_Optimal_CbsNode(node, newConstraint, conflictingAgentIndex);
                }
                else
                {
                    newConstraint = new P_Robust_Optimal_CbsConstraint(conflict, instance, doLeftChild, constraintPolicy, conflictRange, true);
                    child = new P_Robust_Optimal_CbsNode(node, newConstraint, conflictingAgentIndex, conflictingAgent2Index);
                }
                int nnn = child.GetHashCode();
                if (this.doMalte && doLeftChild == false || doThirdChild)
                {
                    ...
                    // don't, as necessary.
                    var newMustConstraint = new P_Robust_Optimal_CbsConstraint(conflict, instance, true, constraintPolicy, conflictRange, true);
                ...
                if (closedList.ContainsKey(child) == false)
                {
                    ...
                    bool success;
                    //if (!doThirdChild)
                        success = child.Replan(conflictingAgentIndex, this.minDepth, null, -1, minCost, conflictingAgent2Index); // The node takes the max between minDepth and the max time over all constraints.
                    if (doThirdChild)//else
                    {
                        success = true;
                        child.totalCost = node.totalCost;
                    }
                    if (success == false)
        ...
        protected bool AdoptConditionally(P_Robust_Optimal_CbsNode node, P_Robust_Optimal_CbsNode adoptionCandidate, ushort nodeOrigH)
        {
            if (debug)
    ...
    /// </summary>
    public class P_Robust_Optimal_CBS_GlobalConflicts : P_Robust_Optimal_CBS_LocalConflicts
    {
        public int[][] globalConflictsCounter;

        public P_Robust_Optimal_CBS_GlobalConflicts(ICbsSolver singleAgentSolver, ICbsSolver generalSolver, int mergeThreshold = -1,
                                  bool doShuffle = false, BypassStrategy bypassStrategy = BypassStrategy.NONE, bool doMalte = false,
                                  ConflictChoice conflictChoice = ConflictChoice.FIRST,
                                  bool justbreakForConflicts = false, bool useMddHeuristic = false,
                                  int lookaheadMaxExpansions = int.MaxValue, bool mergeCausesRestart = false, int conflictRange = 0, Run.ConstraintPolicy constraintPolicy = Run.ConstraintPolicy.Single, double probust_prob = 0, double  delay_prob = 0, bool pRobust_Empirically = false)
            : base(singleAgentSolver, generalSolver, mergeThreshold, doShuffle, bypassStrategy, doMalte, conflictChoice,
                   justbreakForConflicts, useMddHeuristic, lookaheadMaxExpansions, mergeCausesRestart, conflictRange, constraintPolicy, probust_prob, delay_prob, pRobust_Empirically)
        {
        ...
        protected override bool ShouldMerge(P_Robust_Optimal_CbsNode node)
        {
