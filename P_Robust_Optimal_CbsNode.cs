...
namespace CPF_experiment
{
    ...
    public class P_Robust_Optimal_CbsNode : IComparable<IBinaryHeapItem>, IBinaryHeapItem
    {
        ...
        public double p_zero = -1;
        private int binaryHeapIndex;
        public CbsConflict conflict;
        public P_Robust_Optimal_CbsConstraint constraint;
        /// <summary>
        /// Forcing an agent to be at a certain place at a certain time
        /// </summary>
        P_Robust_Optimal_CbsConstraint mustConstraint;
        public P_Robust_Optimal_CbsNode prev;
        public ushort depth;
        ...
        protected P_Robust_Optimal_CBS_LocalConflicts cbs;
        ...
        public P_Robust_Optimal_CbsNode(int numberOfAgents, ICbsSolver solver, ICbsSolver singleAgentSolver, P_Robust_Optimal_CBS_LocalConflicts cbs, ushort[] agentsGroupAssignment = null)
        {
            this.cbs = cbs;
        ...
        public int agentToReplan2; //for p-robust mest constraint
        ...
        public P_Robust_Optimal_CbsNode(P_Robust_Optimal_CbsNode father, P_Robust_Optimal_CbsConstraint newConstraint, int agentToReplan, int agentToReplan2 = -1)
        {
            this.agentToReplan = agentToReplan;
            this.agentToReplan2 = agentToReplan2;
            ...
            if (agentToReplan2 != -1)   // possitive constraint - delete this conflict, it must be exist in the solution
            {
                this.conflictCountsPerAgent[agentToReplan][agentToReplan2]--;
                if (this.conflictCountsPerAgent[agentToReplan][agentToReplan2] == 0)
                {
                    this.conflictCountsPerAgent[agentToReplan].Remove(agentToReplan2);
                }
                this.conflictCountsPerAgent[agentToReplan2][agentToReplan]--;
                if (this.conflictCountsPerAgent[agentToReplan2][agentToReplan] == 0)
                {
                    this.conflictCountsPerAgent[agentToReplan2].Remove(agentToReplan);
                }

                int indexOfConflictAgent1 = -1;
                int indexOfConflictAgent2 = -1;
                for (int conflictIndex = 0; conflictIndex < this.conflictTimesBiasPerAgent[agentToReplan][agentToReplan2].Count; conflictIndex++)
                {
                    if (this.conflictTimesPerAgent[agentToReplan][agentToReplan2][conflictIndex] == newConstraint.move2.time &&
                        this.conflictTimesBiasPerAgent[agentToReplan][agentToReplan2][conflictIndex] == newConstraint.move.time - newConstraint.move2.time)
                        indexOfConflictAgent1 = conflictIndex;
                }

                for (int conflictIndex = 0; conflictIndex < this.conflictTimesBiasPerAgent[agentToReplan2][agentToReplan].Count; conflictIndex++)
                {
                    if (this.conflictTimesPerAgent[agentToReplan2][agentToReplan][conflictIndex] == newConstraint.move.time &&
                        this.conflictTimesBiasPerAgent[agentToReplan2][agentToReplan][conflictIndex] == newConstraint.move2.time - newConstraint.move.time)
                        indexOfConflictAgent2 = conflictIndex;
                }


                this.conflictTimesPerAgent[agentToReplan][agentToReplan2].RemoveAt(indexOfConflictAgent1);
                if (this.conflictTimesPerAgent[agentToReplan][agentToReplan2].Count == 0)
                {
                    this.conflictTimesPerAgent[agentToReplan].Remove(agentToReplan2);
                }
                this.conflictTimesPerAgent[agentToReplan2][agentToReplan].RemoveAt(indexOfConflictAgent2);
                if (this.conflictTimesPerAgent[agentToReplan2][agentToReplan].Count == 0)
                {
                    this.conflictTimesPerAgent[agentToReplan2].Remove(agentToReplan);
                }

                this.conflictTimesBiasPerAgent[agentToReplan][agentToReplan2].RemoveAt(indexOfConflictAgent1);
                if (this.conflictTimesBiasPerAgent[agentToReplan][agentToReplan2].Count == 0)
                {
                    this.conflictTimesBiasPerAgent[agentToReplan].Remove(agentToReplan2);
                }
                this.conflictTimesBiasPerAgent[agentToReplan2][agentToReplan].RemoveAt(indexOfConflictAgent2);
                if (this.conflictTimesBiasPerAgent[agentToReplan2][agentToReplan].Count == 0)
                {
                    this.conflictTimesBiasPerAgent[agentToReplan2].Remove(agentToReplan);
                }
 
                this.conflictProbabilityPerAgent[agentToReplan][agentToReplan2].RemoveAt(indexOfConflictAgent1);
                if (this.conflictProbabilityPerAgent[agentToReplan][agentToReplan2].Count == 0)
                {
                    this.conflictProbabilityPerAgent[agentToReplan].Remove(agentToReplan2);
                }
                this.conflictProbabilityPerAgent[agentToReplan2][agentToReplan].RemoveAt(indexOfConflictAgent2);
                if (this.conflictProbabilityPerAgent[agentToReplan2][agentToReplan].Count == 0)
                {
                    this.conflictProbabilityPerAgent[agentToReplan2].Remove(agentToReplan);
                }
            }
        ...
        public P_Robust_Optimal_CbsNode(P_Robust_Optimal_CbsNode father, int mergeGroupA, int mergeGroupB)
        {
            this.allSingleAgentPlans = father.allSingleAgentPlans.ToArray<SinglePlan>();
            this.allSingleAgentCosts = father.allSingleAgentCosts.ToArray<int>();
        ...
        public bool Solve(int depthToReplan)
        {
            ...
            HashSet<P_Robust_Optimal_CbsConstraint> newConstraints = this.GetConstraints(); // Probably empty as this is probably the root of the CT.
            var CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT];
            var constraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];

            HashSet_U<P_Robust_Optimal_CbsConstraint> mustConstraints = null;
            HashSet<P_Robust_Optimal_CbsConstraint> newMustConstraints = null;
            Dictionary<int,int> agentsWithMustConstraints = null;
            if (problem.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS))
            {
                mustConstraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
                newMustConstraints = this.GetMustConstraints();
                agentsWithMustConstraints = mustConstraints.Select<P_Robust_Optimal_CbsConstraint, int>(constraint => constraint.agentNum).Distinct().ToDictionary<int, int>(x => x); // ToDictionary because there's no ToSet...
            }

            Dictionary<int, int> agentsWithConstraints = null;
            if (constraints.Count != 0)
            {
                int maxConstraintTimeStep = constraints.Max<P_Robust_Optimal_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
                agentsWithConstraints = constraints.Select<P_Robust_Optimal_CbsConstraint, int>(constraint => constraint.agentNum).Distinct().ToDictionary<int, int>(x => x); // ToDictionary because there's no ToSet...
            }
            ...
            Dictionary<Tuple<int, int, Move, Move>, double> collisionsDictionary = null;
            if (getConflictRange() == -1)
            {
                if (this.isPRobustGoal())
                {
                    LinkedList<List<Move>> currentList = singleAgentPathsToList(allSingleAgentPlans);
                    this.isGoal = this.cbs.runner.isGoalCalculation(problem, cbs, PRobustBound, currentList, out collisionsDictionary, cbs.pRobust_Empirically);
                }
            }
                        ...
                        this.agentNumToIndex[pair.Key] < i)                                 // Just an optimization. Would also be correct without this check.
                    {
                        ...
                        if(this.conflictTimesPerAgent[i][pair.Key] != null)
                        {
                            ...
                            for(int f = 0; f < this.conflictTimesPerAgent[this.agentNumToIndex[pair.Key]][problem.m_vAgents[i].agent.agentNum].Count; f++)
                            {
                                ...
                                int agentA = i;
                                int agentB = pair.Key;
                                int agentAtime = this.conflictTimesPerAgent[this.agentNumToIndex[pair.Key]]
                                [problem.m_vAgents[i].agent.agentNum][f] + this.conflictTimesBiasPerAgent[this.agentNumToIndex[pair.Key]][problem.m_vAgents[i].agent.agentNum][f];
                                int agentBtime = this.conflictTimesPerAgent[this.agentNumToIndex[pair.Key]]
                                [problem.m_vAgents[i].agent.agentNum][f];

                                if (getConflictRange() == -1)
                                { 
                                Move agentAMove = new Move(allSingleAgentPlans[agentA].GetLocationAt(agentAtime));
                                agentAMove.direction = Move.Direction.NO_DIRECTION;

                                Move agentBMove = new Move(allSingleAgentPlans[agentB].GetLocationAt(agentBtime));
                                agentBMove.direction = Move.Direction.NO_DIRECTION;

                                TimedMove agentATMove = new TimedMove(agentAMove, agentAtime);
                                TimedMove agentBTMove = new TimedMove(agentBMove, agentBtime);

                                Tuple<int, int, Move, Move> conflict1 = new Tuple<int, int, Move, Move>(agentA, agentB, agentATMove, agentBTMove);
                                Tuple<int, int, Move, Move> conflict2 = new Tuple<int, int, Move, Move>(agentB, agentA, agentBTMove, agentATMove);
                                    if (collisionsDictionary.Keys.Contains(conflict1) || collisionsDictionary.Keys.Contains(conflict2))
                                    {
                                        double probability = 0;
                                        if (collisionsDictionary.Keys.Contains(conflict1))
                                        {
                                            probability = collisionsDictionary[conflict1];
                                            collisionsDictionary.Remove(conflict1);
                                        }
                                        else
                                        {
                                            probability = collisionsDictionary[conflict2];
                                            collisionsDictionary.Remove(conflict2);
                                        }

                                        this.conflictProbabilityPerAgent[i][pair.Key][f] = probability;

                                        if (this.conflictProbabilityPerAgent[this.agentNumToIndex[pair.Key]]
                                            [problem.m_vAgents[i].agent.agentNum] == null)
                                            this.conflictProbabilityPerAgent[this.agentNumToIndex[pair.Key]]
                                                [problem.m_vAgents[i].agent.agentNum] = new List<double>(this.conflictProbabilityPerAgent[i][pair.Key]);
                                        else
                                            this.conflictProbabilityPerAgent[this.agentNumToIndex[pair.Key]]
                                                [problem.m_vAgents[i].agent.agentNum][f] = probability;
        ...
        /// <returns></returns>
        public bool Replan(int agentForReplan, int depthToReplan, List<AgentState> subGroup = null, int maxPlanSize = -1, int minCost = -1, int agentForReplan2 = -1)
        {
            ProblemInstance problem = this.cbs.GetProblemInstance();
            Dictionary_U<TimedMove, int> CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT];
            ...
            HashSet<P_Robust_Optimal_CbsConstraint> newConstraints = this.GetConstraints();
            var constraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];
            ...
            if (problem.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS))
            {
                //mustConstraints = new HashSet_U<CbsConstraint>();
                mustConstraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
                newMustConstraints = this.GetMustConstraints();
            }
            ...
            IEnumerable<P_Robust_Optimal_CbsConstraint> myConstraints = constraints.Where<P_Robust_Optimal_CbsConstraint>(constraint => subGroupAgentNums.ContainsKey(constraint.agentNum)); // TODO: Consider passing only myConstraints to the low level to speed things up.
            if (myConstraints.Count<P_Robust_Optimal_CbsConstraint>() != 0)
            {
                int maxConstraintTimeStep = myConstraints.Max<P_Robust_Optimal_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
            }
            if (mustConstraints != null)
            {
                IEnumerable<P_Robust_Optimal_CbsConstraint> myMustConstraints = mustConstraints.Where<P_Robust_Optimal_CbsConstraint>(constraint => subGroupAgentNums.ContainsKey(constraint.agentNum));
                if (myMustConstraints.Count<P_Robust_Optimal_CbsConstraint>() != 0)
                {
                    int maxMustConstraintTimeStep = myMustConstraints.Max<P_Robust_Optimal_CbsConstraint>(constraint => constraint.time);
                    depthToReplan = Math.Max(depthToReplan, maxMustConstraintTimeStep); // Give all must constraints a chance to affect the plan
                }
            }

            /*bool        agentHasMustConstraint  = false;    // new for must constraints in p-robust
            TimedMove   newStart                = (subGroup.ToArray())[0].lastMove;
            Move        newGoal                 = (subGroup.ToArray())[0].agent.Goal;
            int         currentTime             = constraint.time;
            int         newStartTime            = 0;
            int         newGoalTime             = int.MaxValue;
            foreach (CbsConstraint singleConstraint in constraints)
            {
                if (singleConstraint.agentNum == agentForReplan &&
                    singleConstraint.mustConstraint == true)
                {
                    agentHasMustConstraint = true;
                    if (singleConstraint.time < currentTime  &&
                        singleConstraint.time > newStartTime)
                    {
                        newStartTime    = singleConstraint.time;
                        newStart        = new TimedMove(singleConstraint.move, 0);
                    }
                    else if (singleConstraint.time > currentTime &&
                             singleConstraint.time < newGoalTime)
                    {
                        newGoalTime         = singleConstraint.time;
                        newGoal             = new Move(singleConstraint.move);
                        newGoal.direction   = Move.Direction.NO_DIRECTION;
                    }
                    else if (singleConstraint.time      ==  currentTime         &&
                             singleConstraint.move.x    ==  constraint.move.x   &&
                             singleConstraint.move.y    ==  constraint.move.y)
                        return false;
                }
            }
            bool solved = false;
            var sic = new SumIndividualCosts();
            var astar = new ClassicAStar(sic, false, false, -1); ;
            if (agentHasMustConstraint)
            {*/
            /*AgentState   curentAgentState = new AgentState(newStart.x, newStart.y, newGoal.x, newGoal.y, agentForReplan);
            curentAgentState.lastMove.time = newStartTime;
            AgentState[] newAgentState = new AgentState[problem.m_vAgents.Count()];
            for (int agent = 0; agent < problem.m_vAgents.Count(); agent++)
            {
                if (agent == agentForReplan)
                    newAgentState[agent] = curentAgentState;
                else
                    newAgentState[agent] = problem.m_vAgents[agent];
            }
            subProblem = problem.Subproblem(newAgentState);
            Move[][]    oldOptimalMoves = problem.singleAgentOptimalMoves;
            int[][]     oldOptimalCosts = problem.singleAgentOptimalCosts;
            subProblem.ComputeSingleAgentShortestPaths();    // out to calculate only a single plan!!!

            ProblemInstance newProblem = new ProblemInstance();*/
            /*if (agentForReplan != 0)
            {
                subProblem.singleAgentOptimalCosts.ad
                subProblem.singleAgentOptimalCosts[agentForReplan] = subProblem.singleAgentOptimalCosts[0];
                subProblem.singleAgentOptimalCosts[0] = null;
                subProblem.singleAgentOptimalMoves[agentForReplan] = subProblem.singleAgentOptimalMoves[0];
                subProblem.singleAgentOptimalMoves[0] = null;
            }*/
            bool solved = false;
            if (agentForReplan2 == -1)
            {
                relevantSolver.Setup(subProblem, depthToReplan, this.cbs.runner, minCost);
                solved = relevantSolver.Solve();

                /*List<uint> listOfAgents = new List<uint>() { (uint)agentForReplan };
                sic.init(subProblem, listOfAgents);
                astar.Setup(subProblem, -1, this.cbs.runner, minCost);
                solved = astar.Solve();*/
                // }
                /*else
                {*/
            ...
            SinglePlan[] singlePlans;
            int[] singleCosts;
            Dictionary<int, int> perAgent;
            Dictionary<int, List<int>> conflictTimes;
            Dictionary<int, List<int>> conflictTimesBias;
            Dictionary<int, List<double>> conflictProbability; // to imp!!!!!

            // Copy the SinglePlans for the solved agent group from the solver to the appropriate places in this.allSingleAgentPlans
            //if (!agentHasMustConstraint)
            //{
                singlePlans         = relevantSolver.GetSinglePlans();
                singleCosts         = relevantSolver.GetSingleCosts();
                perAgent            = relevantSolver.GetExternalConflictCounts();
                conflictTimes       = relevantSolver.GetConflictTimes();
                conflictTimesBias   = relevantSolver.GetConflictTimesBias();
                conflictProbability = relevantSolver.GetConflictProbability(); // to imp!!!!!
            //}
            /*else
            {
                SinglePlan[] partSinglePlans = astar.GetSinglePlans(); ;
                int[] partSingleCosts = astar.GetSingleCosts(); ;
                Dictionary<int, int> partPerAgen = astar.GetExternalConflictCounts();
                Dictionary<int, List<int>> partConflictTimes = astar.GetConflictTimes();
                Dictionary<int, List<int>> partConflictTimesBias = astar.GetConflictTimesBias();
                Dictionary<int, List<double>> partConflictProbability = astar.GetConflictProbability(); // to imp!!!!!
            ...
            {
                if (this.isPRobustGoal())
                    this.isGoal = this.cbs.runner.isGoalCalculation(problem, cbs, PRobustBound, singleAgentPathsToList(allSingleAgentPlans), out collisionsDictionary, cbs.pRobust_Empirically);
               /* p_zero = 0;
                foreach (double value in collisionsDictionary.Values)
                    p_zero += value;
                p_zero = 1 - p_zero;*/
            }
            ...
            if (underSolve == false)
            {
                ...
                // update conflicts probabilities
                if (getConflictRange() == -1)
                {
                    for (int i = 0; i < this.conflictCountsPerAgent.Length; i++)
                        for (int j = 0; j < this.conflictCountsPerAgent.Length; j++)
                            if(this.conflictTimesPerAgent[this.agentNumToIndex[j]].Keys.Contains(problem.m_vAgents[i].agent.agentNum))
                            for (int f = 0; f < this.conflictTimesPerAgent[this.agentNumToIndex[j]][problem.m_vAgents[i].agent.agentNum].Count; f++)
                    {
                        int agentA = i;
                        int agentB = j;
                        int agentAtime = this.conflictTimesPerAgent[this.agentNumToIndex[agentB]]
                        [problem.m_vAgents[i].agent.agentNum][f] + this.conflictTimesBiasPerAgent[this.agentNumToIndex[agentB]][problem.m_vAgents[i].agent.agentNum][f];
                        int agentBtime = this.conflictTimesPerAgent[this.agentNumToIndex[agentB]]
                        [problem.m_vAgents[i].agent.agentNum][f];

                        allSingleAgentPlans[agentA].GetLocationAt(agentAtime);
                        ...
                        if (collisionsDictionary.Keys.Contains(conflict1) || collisionsDictionary.Keys.Contains(conflict2))
                        {
                            ...
                            //if (this.conflictProbabilityPerAgent[i][j].Count <= f)
                              //  continue;
                            ...
                            this.conflictProbabilityPerAgent[i][j][f] = probability;

                            if (this.conflictProbabilityPerAgent[this.agentNumToIndex[j]]
                                [problem.m_vAgents[i].agent.agentNum] == null)
                                this.conflictProbabilityPerAgent[this.agentNumToIndex[j]]
                                    [problem.m_vAgents[i].agent.agentNum] = new List<double>(this.conflictProbabilityPerAgent[i][j]);
                            else
                                this.conflictProbabilityPerAgent[this.agentNumToIndex[j]]
                                    [problem.m_vAgents[i].agent.agentNum][f] = probability;
               ...
               /* if (getConflictRange() == -1 && collisionsDictionary.Count != 0)
                    return false;*/
                /*else
        ...
        public void Print()
        {
            ...
            IList<P_Robust_Optimal_CbsConstraint> constraints = this.GetConstraintsOrdered();
            Debug.WriteLine(constraints.Count.ToString() + " relevant internal constraints so far: ");
            foreach (P_Robust_Optimal_CbsConstraint constraint in constraints)
            {
                Debug.WriteLine(constraint);
            }
            ISet<P_Robust_Optimal_CbsConstraint> mustConstraints = this.GetMustConstraints(); // TODO: Ordered
            Debug.WriteLine(mustConstraints.Count.ToString() + " relevant internal must constraints so far: ");
            foreach (P_Robust_Optimal_CbsConstraint mustConstraint in mustConstraints)
            {
                Debug.WriteLine(mustConstraint);
            }
            ProblemInstance problem = this.cbs.GetProblemInstance();
            var externalConstraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];
            Debug.WriteLine(externalConstraints.Count.ToString() + " external constraints: ");
            foreach (P_Robust_Optimal_CbsConstraint constraint in externalConstraints)
            {
                Debug.WriteLine(constraint);
            }
            ...
            if (this.cbs.GetType() == typeof(P_Robust_Optimal_CBS_GlobalConflicts) && this.cbs.mergeThreshold != -1)
            {
                for (int i = 0; i < ((P_Robust_Optimal_CBS_GlobalConflicts)this.cbs).globalConflictsCounter.Length; i++)
			    {
                    Debug.Write("Agent " + i.ToString() + " global historic conflict counts: ");
                    for (int j = 0; j < i; j++)
                    {
                        Debug.Write("a" + j.ToString() + ":" + ((P_Robust_Optimal_CBS_GlobalConflicts)this.cbs).globalConflictsCounter[i][j] + " ");
                    }
                    Debug.WriteLine("");
			    }
            }
        ...
        protected void UpdateAtGoalConflictCounts(int agentIndex, int maxPlanSize, IReadOnlyDictionary<TimedMove, List<int>> CAT)
        {
            ...
            {
                afterGoal.time = time;
                int timeWithoutDelays = 0;  /*just old
        ...
        /// Luckily, in P_Robust_Optimal_CBS_LocalConflicts, a merge is only possible once every B+1 depth steps,
        ...
        /// 
        /// In P_Robust_Optimal_CBS_GlobalConflicts, we could use the global table to discount some merges.
        /// </summary>
        protected void CalcMinOpsToSolve()
        {
            if (this.cbs.tieBreakForMoreConflictsOnly == false)
            {
                ...
                {
                    if (this.cbs.GetType() == typeof(P_Robust_Optimal_CBS_LocalConflicts))
                    {
        ...
        public void ChooseConflict()
        {
            ...
            int time2;
            if (this.cbs.conflictChoice == P_Robust_Optimal_CBS_LocalConflicts.ConflictChoice.FIRST)
            {
            ...
            else if (this.cbs.conflictChoice == P_Robust_Optimal_CBS_LocalConflicts.ConflictChoice.MOST_CONFLICTING)
            {
                ChooseBestConflict(out groupRepA, out groupRepB, out time);
                this.conflict = FindConflict(groupRepA, groupRepB, time);
            }
            else if (this.cbs.conflictChoice == P_Robust_Optimal_CBS_LocalConflicts.ConflictChoice.CARDINAL_MDD)
            {
            ...
            else if (this.cbs.conflictChoice == P_Robust_Optimal_CBS_LocalConflicts.ConflictChoice.CARDINAL_LOOKAHEAD)
            {
        ...
        private bool buildMddForAgent(int conflictingIndex)
        {
            if (this.mdds[conflictingIndex] == null)
            {
                ...
                IEnumerable<P_Robust_Optimal_CbsConstraint> myConstraints = constraints.Where<P_Robust_Optimal_CbsConstraint>(
                ...
                if (mustConstraints != null)
                {
                    IEnumerable<P_Robust_Optimal_CbsConstraint> myMustConstraints = mustConstraints.Where<P_Robust_Optimal_CbsConstraint>(
                        constraint => constraint.agentNum == problem.m_vAgents[conflictingIndex].agent.agentNum);
                    if (myMustConstraints.Count<P_Robust_Optimal_CbsConstraint>() != 0)
                    {
                        int maxMustConstraintTimeStep = myMustConstraints.Max<P_Robust_Optimal_CbsConstraint>(constraint => constraint.time);
        ...
        //    P_Robust_Optimal_AgentToCheckForCardinalConflicts[] agentsData = new P_Robust_Optimal_AgentToCheckForCardinalConflicts[this.problem.m_vAgents.Length];
        //    int[] groupSizes = this.GetGroupSizes();
        //    for (int i = 0; i < agentsData.Length; i++)
        //    {
        //        agentsData[i] = new P_Robust_Optimal_AgentToCheckForCardinalConflicts(
        //            /*this.mdds[i] != null,
        ...
        //    while (toCheck.Count != 0)
        //    {
        //        int i = ((P_Robust_Optimal_AgentToCheckForCardinalConflicts) toCheck.Remove()).index;
        ...
        //            this.conflictTimesPerAgent[i].Keys.Select<int, P_Robust_Optimal_AgentToCheckForCardinalConflicts>(
        //                num => agentsData[this.agentNumToIndex[num]]
        //            ).ToList<P_Robust_Optimal_AgentToCheckForCardinalConflicts>());

        //        while (otherAgents.Count != 0)
        //        {
        //            int otherAgentIndex = ((P_Robust_Optimal_AgentToCheckForCardinalConflicts) otherAgents.Remove()).index;
                ...
                for (int i = 0; i < this.conflictTimesPerAgent.Length; i++)
                {
                    foreach (var otherAgentNumAndConflictTimes in this.conflictTimesPerAgent[i])
                    {
                        for (int conflictTimeIndex = 0; conflictTimeIndex < otherAgentNumAndConflictTimes.Value.Count; conflictTimeIndex++)
                        {
                            ...
                            int conflictTime2   = conflictTime + this.conflictTimesBiasPerAgent[i][otherAgentNumAndConflictTimes.Key][conflictTimeIndex];
                            if (gap > Math.Abs(conflictTime - conflictTime2)    ||
                                (gap == Math.Abs(conflictTime - conflictTime2)  && conflictTime < time))
                            //if (conflictTime < time)   
                            {
                                gap = Math.Abs(conflictTime - conflictTime2);
                                time = conflictTime;    
        ...
        //    int firstSmallestGroupedMostConflictingAgentIndex = smallestGroupedMostConflictingAgentIndices.Min(); // Optimalally tie-breaking is important.
        ...
        //    int firstSmallestGroupedMostConflictingAmongThem = smallestGroupedAmongThemMostConflictingAmongThem.Min(); // Optimalally tie-breaking.
        ...
        public void AdoptSolutionOf(P_Robust_Optimal_CbsNode child)
        {
            Debug.Assert(this.totalCost == child.totalCost, "Tried to adopt node of a different cost");
            this.agentAExpansion = P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED;
            this.agentBExpansion = P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED;
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                HashSet<P_Robust_Optimal_CbsConstraint> constraints = this.GetConstraints();
                ...
                constraints = this.GetMustConstraints();
                foreach (P_Robust_Optimal_CbsConstraint constraint in constraints)
                {
                    ans += constraint.GetHashCode();
                }
        ...
        {
            P_Robust_Optimal_CbsNode other = (P_Robust_Optimal_CbsNode)obj;

            if (this.agentsGroupAssignment.SequenceEqual<ushort>(other.agentsGroupAssignment) == false)
                return false;

            P_Robust_Optimal_CbsNode current = this;
            HashSet<P_Robust_Optimal_CbsConstraint> other_constraints = other.GetConstraints();
            HashSet<P_Robust_Optimal_CbsConstraint> constraints = this.GetConstraints();
        ...
        {
            P_Robust_Optimal_CbsNode other = (P_Robust_Optimal_CbsNode)item;
        ...
        public int CompareToPRobust(P_Robust_Optimal_CbsNode other)  //for p-robust
        {
            if (this.p_zero == other.p_zero)
                return 0;
            if (this.p_zero > other.p_zero)
                return -1;
            else
        ...
        public int CompareToIgnoreH(P_Robust_Optimal_CbsNode other, bool ignorePartialExpansion = false)
        {
            // Tie breaking:
            ...
            if (ignorePartialExpansion == false)
            {
                ...
                if ((this.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED || this.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED) &&
                    other.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED && other.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED)
                    return -1;
                if ((other.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED || other.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.DEFERRED) &&
                    this.agentAExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED && this.agentBExpansion == P_Robust_Optimal_CbsNode.ExpansionState.NOT_EXPANDED)
                    return 1;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public P_Robust_Optimal_CbsConstraint GetLastConstraint()
        {
            return this.constraint;
        }

        public HashSet<P_Robust_Optimal_CbsConstraint> GetConstraints()
        {
            var constraints = new HashSet<P_Robust_Optimal_CbsConstraint>();
            int currentRange = 0;
            P_Robust_Optimal_CbsNode current = this;
                ...
                if (current.constraint != null && // Next check not enough if "surprise merges" happen (merges taken from adopted child)
                    ...
                    for (int i = -currentRange; i <= currentRange; i++)
                    {
                        ...
                        if (cbs.constraintPolicy == Run.ConstraintPolicy.DoubleRange && isLeftNode(current) && i != 0)
                            continue;
                        P_Robust_Optimal_CbsConstraint currentConstraint = current.constraint;
                        TimedMove     currentMove       = current.constraint.move;
                        P_Robust_Optimal_CbsConstraint newConstraint = new P_Robust_Optimal_CbsConstraint(currentConstraint.agentNum, currentMove.x, currentMove.y, currentMove.direction, currentMove.time + i);
                        if (currentConstraint.mustConstraint)
                        {
                            /*TimedMove currentMove2 = current.constraint.move2;
                            newConstraint = new CbsConstraint(currentConstraint.agentNum, currentMove.x, currentMove.y, currentMove.direction, currentMove.time + i, 0, currentConstraint.agentNum2, currentConstraint.move2.x, currentConstraint.move2.y, currentConstraint.move2.direction, currentConstraint.move2.time);
                            constraints.Add(newConstraint);
                            newConstraint = new CbsConstraint(currentConstraint.agentNum2, currentMove2.x, currentMove2.y, currentMove2.direction, currentMove2.time + i, 0, currentConstraint.agentNum, currentConstraint.move.x, currentConstraint.move.y, currentConstraint.move.direction, currentConstraint.move.time);
                            constraints.Add(newConstraint);*/
                        }
                        else
                            constraints.Add(newConstraint);
        ...
        private bool isLeftNode(P_Robust_Optimal_CbsNode node)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<P_Robust_Optimal_CbsConstraint> GetConstraintsOrdered()
        {
            var constraints = new List<P_Robust_Optimal_CbsConstraint>();
            P_Robust_Optimal_CbsNode current = this;
            while (current.depth > 0) // The root has no constraints
            {
                if (current.constraint != null && // Next check not enough if "surprise merges" happen (merges taken from adopted child)
        ...
        public HashSet<P_Robust_Optimal_CbsConstraint> GetMustConstraints()
        {
            var constraints = new HashSet<P_Robust_Optimal_CbsConstraint>();
            P_Robust_Optimal_CbsNode current = this;
            while (current.depth > 0)
            {
                if (current.mustConstraint != null) // TODO: Ignore must constraint from merged agents
                {
                    ...
                    P_Robust_Optimal_CbsConstraint oneConstraint = current.mustConstraint;
                    constraints.Add(new P_Robust_Optimal_CbsConstraint(oneConstraint.agentNum2, oneConstraint.move2, -1));
                }
                current = current.prev;
            }
        ...
        public bool ShouldMerge(int mergeThreshold)
        {
           ...
           //     return false;
            P_Robust_Optimal_CbsNode current = this.prev;
            int a, b;
            while (current != null)
            {
        ...
        public bool DoesMustConstraintAllow(P_Robust_Optimal_CbsConstraint check)
        {
            P_Robust_Optimal_CbsNode current = this;
            while (current != null)
            {
        ...
        public void SetMustConstraint(P_Robust_Optimal_CbsConstraint set)
        {
        ...
        public bool Replan3b(int agentForReplan, int depthToReplan, int minCost = -1)
        {
            ...
            var CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT];
            var constraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];
            var mustConstraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problem.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
            HashSet<P_Robust_Optimal_CbsConstraint> newMustConstraints = this.GetMustConstraints();

            if (newConstraints.Count != 0)
            {
                int maxConstraintTimeStep = newConstraints.Max<P_Robust_Optimal_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
            }
    ...
    /// </summary>
    public class P_Robust_Optimal_AgentToCheckForCardinalConflicts : IBinaryHeapItem
    {
        //public bool hasMDD;
        ...
        public P_Robust_Optimal_AgentToCheckForCardinalConflicts(/*bool hasMDD, int conflictingAgentsWithMDD,*/ int groupSize, int degree,
        ...
        public int CompareTo(IBinaryHeapItem item)
        {
            P_Robust_Optimal_AgentToCheckForCardinalConflicts other = (P_Robust_Optimal_AgentToCheckForCardinalConflicts)item;
