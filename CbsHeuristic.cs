...
namespace CPF_experiment
{
    ...
    class CbsHeuristic : HeuristicCalculator
    {
        ...
        /// </param>
        public CbsHeuristic(P_Robust_Optimal_CBS_LocalConflicts cbs, Run runner, bool reportSolution = false, int minAboveSic = 1, bool validate = false)
        {
            this.cbs = cbs;
            this.runner = runner;
        ...
        protected uint h(WorldState s, int targetCost, int sicEstimate=-1, int lowLevelGeneratedCap=-1, int milliCap=int.MaxValue, bool resume=false)
        {
            ...
            if (resume == false)
            {
                ...
                if (this.cbs.openList.Count > 0 && this.cbs.topMost)
                {
                    ...
                    Debug.Assert(((P_Robust_Optimal_CbsNode)this.cbs.openList.Peek()).totalCost - s.g == (int)sicEstimate,
    ...
    {
        public DyanamicLazyCbsh(P_Robust_Optimal_CBS_LocalConflicts cbs, Run runner, bool reportSolution = false, bool validate = false)
