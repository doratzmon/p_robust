...
{   
    ...
    /// </summary>
    public class P_Robust_Optimal_ClassicAStar : ICbsSolver, IMStarSolver
    {
        ...
        protected HashSet_U<P_Robust_Optimal_CbsConstraint> constraints;
        /// <summary>
        ...
        protected Dictionary<WorldState, HashSet<P_Robust_Optimal_CbsConstraint>[]> mstarPlanBasesToTheirConstraints; // Most nodes won't have constraints so I don't want to make their memory footprint needlessly large.
        ...
        /// Default constructor.
        /// </summary>
        public P_Robust_Optimal_ClassicAStar(HeuristicCalculator heuristic = null, bool mStar = false, bool mStarShuffle = false, int conflictRange = 0)
        {
            this.closedList = new Dictionary<WorldState, WorldState>();
            this.openList = new OpenList(this);
            this.heuristic = heuristic;
            
            this.queryConstraint = new P_Robust_Optimal_CbsConstraint();
        ...
        public virtual void Setup(ProblemInstance problemInstance, int minDepth, Run runner, int minCost)
        {
            ...
            if (problemInstance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS) &&
                ((HashSet_U<P_Robust_Optimal_CbsConstraint>)problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS]).Count != 0)
                 this.constraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];
 
             if (problemInstance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS) &&
                 ((HashSet_U<P_Robust_Optimal_CbsConstraint>)problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS]).Count != 0)
             {
                //this.mustConstraints = //dddaaa
                 var musts = (HashSet_U<P_Robust_Optimal_CbsConstraint>)problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
                 this.mustConstraints = new Dictionary<int, TimedMove>[musts.Max<P_Robust_Optimal_CbsConstraint>(con => con.GetTimeStep()) + 1]; // To have index MAX, array needs MAX + 1 places.
                 foreach (P_Robust_Optimal_CbsConstraint con in musts)
                 {
        ...
        /// </summary>
        private P_Robust_Optimal_CbsConstraint queryConstraint;
        ...
        protected virtual bool ProcessGeneratedNode(WorldState currentNode)
        {
            ...
            // within the budget
            {
                ...
                if (instance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.CAT))
                {
                    // Accumulating the conflicts count from parent to child.
                    // We're counting conflicts along the entire path, so the parent's conflicts count is added to the child's:
                    currentNode.cbsInternalConflicts = new Dictionary<int,int>(currentNode.prevStep.cbsInternalConflicts);
                    ...
                    currentNode.UpdateConflictCounts(
                        ((IReadOnlyDictionary<TimedMove, List<int>>)instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT]), conflictRange);
