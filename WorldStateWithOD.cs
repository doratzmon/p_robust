    ...
    public class WorldStateWithOD : WorldState
    {
        ...
        public override ProblemInstance ToProblemInstance(ProblemInstance initial)
        {
            ...
            if (this.agentTurn != 0)
            {
                ...
                if (subproblem.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS) == false)
                    subproblem.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS] = new HashSet_U<P_Robust_Optimal_CbsConstraint>();
                var mustConstraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)subproblem.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
                var newMustConstraints = new HashSet<P_Robust_Optimal_CbsConstraint>();
                for (int i = 0; i < this.agentTurn; ++i)
                {
                    newMustConstraints.Add(new P_Robust_Optimal_CbsConstraint(this.allAgentsState[i].agent.agentNum, this.allAgentsState[i].lastMove));
                }
