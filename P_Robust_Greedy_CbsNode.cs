...
namespace CPF_experiment
{
    ...
    public class P_Robust_Greedy_CbsNode : IComparable<IBinaryHeapItem>, IBinaryHeapItem
    {
        ...
        public P_Robust_Greedy_CbsConstraint constraint;
        /// <summary>
        /// Forcing an agent to be at a certain place at a certain time
        /// </summary>
        P_Robust_Greedy_CbsConstraint mustConstraint;
        public P_Robust_Greedy_CbsNode prev;
        public ushort depth;
        ...
        protected P_Robust_Greedy_CBS_LocalConflicts cbs;
        ...
        public P_Robust_Greedy_CbsNode(int numberOfAgents, ICbsSolver solver, ICbsSolver singleAgentSolver, P_Robust_Greedy_CBS_LocalConflicts cbs, ushort[] agentsGroupAssignment = null)
        {
            this.cbs = cbs;
        ...
        public P_Robust_Greedy_CbsNode(P_Robust_Greedy_CbsNode father, P_Robust_Greedy_CbsConstraint newConstraint, int agentToReplan)
        {
        ...
        public P_Robust_Greedy_CbsNode(P_Robust_Greedy_CbsNode father, int mergeGroupA, int mergeGroupB)
        {
            this.allSingleAgentPlans = father.allSingleAgentPlans.ToArray<SinglePlan>();
            this.allSingleAgentCosts = father.allSingleAgentCosts.ToArray<int>();
        ...
        public bool Solve(int depthToReplan)
        {
            ...
            HashSet<P_Robust_Greedy_CbsConstraint> newConstraints = this.GetConstraints(); // Probably empty as this is probably the root of the CT.
            var CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CAT];
            var constraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];

            HashSet_U<P_Robust_Greedy_CbsConstraint> mustConstraints = null;
            HashSet<P_Robust_Greedy_CbsConstraint> newMustConstraints = null;
            Dictionary<int,int> agentsWithMustConstraints = null;
           /* if (problem.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS))
            {
                mustConstraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS];
                newMustConstraints = this.GetMustConstraints();
                agentsWithMustConstraints = mustConstraints.Select<P_Robust_Greedy_CbsConstraint, int>(constraint => constraint.agentNum).Distinct().ToDictionary<int, int>(x => x); // ToDictionary because there's no ToSet...
            }*/

            Dictionary<int, int> agentsWithConstraints = null;
            if (constraints.Count != 0)
            {
                int maxConstraintTimeStep = constraints.Max<P_Robust_Greedy_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
                agentsWithConstraints = constraints.Select<P_Robust_Greedy_CbsConstraint, int>(constraint => constraint.agentNum).Distinct().ToDictionary<int, int>(x => x); // ToDictionary because there's no ToSet...
            }
            ...
            if (getConflictRange() == -1)
            {
                ...
                //{
                    ...
                    this.isGoal = this.cbs.runner.isGoalCalculation(problem, cbs, PRobustBound, currentList, out collisionsDictionary, true);
               // }
            }
            else
        ...
        {
            ProblemInstance problem = this.cbs.GetProblemInstance();
            Dictionary_U<TimedMove, int> CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CAT];
            ...
            HashSet<P_Robust_Greedy_CbsConstraint> newConstraints = this.GetConstraints();
            var constraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];
            ...
            IEnumerable<P_Robust_Greedy_CbsConstraint> myConstraints = constraints.Where<P_Robust_Greedy_CbsConstraint>(constraint => subGroupAgentNums.ContainsKey(constraint.agentNum)); // TODO: Consider passing only myConstraints to the low level to speed things up.
            if (myConstraints.Count<P_Robust_Greedy_CbsConstraint>() != 0)
            {
                int maxConstraintTimeStep = myConstraints.Max<P_Robust_Greedy_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
            }
            if (mustConstraints != null)
            {
                IEnumerable<P_Robust_Greedy_CbsConstraint> myMustConstraints = mustConstraints.Where<P_Robust_Greedy_CbsConstraint>(constraint => subGroupAgentNums.ContainsKey(constraint.agentNum));
                if (myMustConstraints.Count<P_Robust_Greedy_CbsConstraint>() != 0)
                {
                    int maxMustConstraintTimeStep = myMustConstraints.Max<P_Robust_Greedy_CbsConstraint>(constraint => constraint.time);
            ...
            if (getConflictRange() == -1)
            {
                Dictionary<Tuple<int, int, Move, Move>, double> collisionsDictionary = null;
                //if (this.isPRobustGoal())
                    this.isGoal = this.cbs.runner.isGoalCalculation(problem, cbs, PRobustBound, singleAgentPathsToList(allSingleAgentPlans), out collisionsDictionary, true );
            }
            else
                this.isGoal = this.countsOfInternalAgentsThatConflict.All(i => i == 0);
        ...
        public void Print()
        {
            ...
            IList<P_Robust_Greedy_CbsConstraint> constraints = this.GetConstraintsOrdered();
            Debug.WriteLine(constraints.Count.ToString() + " relevant internal constraints so far: ");
            foreach (P_Robust_Greedy_CbsConstraint constraint in constraints)
            {
                Debug.WriteLine(constraint);
            }
            ISet<P_Robust_Greedy_CbsConstraint> mustConstraints = this.GetMustConstraints(); // TODO: Ordered
            Debug.WriteLine(mustConstraints.Count.ToString() + " relevant internal must constraints so far: ");
            foreach (P_Robust_Greedy_CbsConstraint mustConstraint in mustConstraints)
            {
                Debug.WriteLine(mustConstraint);
            }
            ProblemInstance problem = this.cbs.GetProblemInstance();
            var externalConstraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];
            Debug.WriteLine(externalConstraints.Count.ToString() + " external constraints: ");
            foreach (P_Robust_Greedy_CbsConstraint constraint in externalConstraints)
            {
                Debug.WriteLine(constraint);
            }
            ...
            if (this.cbs.GetType() == typeof(P_Robust_Greedy_CBS_GlobalConflicts) && this.cbs.mergeThreshold != -1)
            {
                for (int i = 0; i < ((P_Robust_Greedy_CBS_GlobalConflicts)this.cbs).globalConflictsCounter.Length; i++)
			    {
                    Debug.Write("Agent " + i.ToString() + " global historic conflict counts: ");
                    for (int j = 0; j < i; j++)
                    {
                        Debug.Write("a" + j.ToString() + ":" + ((P_Robust_Greedy_CBS_GlobalConflicts)this.cbs).globalConflictsCounter[i][j] + " ");
                    }
                    Debug.WriteLine("");
			    }
            }
        ...
        /// Luckily, in P_Robust_Greedy_CBS_LocalConflicts, a merge is only possible once every B+1 depth steps,
        ...
        /// 
        /// In P_Robust_Greedy_CBS_GlobalConflicts, we could use the global table to discount some merges.
        /// </summary>
        protected void CalcMinOpsToSolve()
        {
            if (this.cbs.tieBreakForMoreConflictsOnly == false)
            {
                ...
                {
                    if (this.cbs.GetType() == typeof(P_Robust_Greedy_CBS_LocalConflicts))
                    {
        ...
        public void ChooseConflict()
        {
            ...
            int time2;
            if (this.cbs.conflictChoice == P_Robust_Greedy_CBS_LocalConflicts.ConflictChoice.FIRST)
            {
            ...
            else if (this.cbs.conflictChoice == P_Robust_Greedy_CBS_LocalConflicts.ConflictChoice.MOST_CONFLICTING)
            {
                ChooseBestConflict(out groupRepA, out groupRepB, out time);
                this.conflict = FindConflict(groupRepA, groupRepB, time);
            }
            else if (this.cbs.conflictChoice == P_Robust_Greedy_CBS_LocalConflicts.ConflictChoice.CARDINAL_MDD)
            {
            ...
            else if (this.cbs.conflictChoice == P_Robust_Greedy_CBS_LocalConflicts.ConflictChoice.CARDINAL_LOOKAHEAD)
            {
        ...
        private bool buildMddForAgent(int conflictingIndex)
        {
            if (this.mdds[conflictingIndex] == null)
            {
                ...
                IEnumerable<P_Robust_Greedy_CbsConstraint> myConstraints = constraints.Where<P_Robust_Greedy_CbsConstraint>(
                ...
                if (mustConstraints != null)
                {
                    IEnumerable<P_Robust_Greedy_CbsConstraint> myMustConstraints = mustConstraints.Where<P_Robust_Greedy_CbsConstraint>(
                        constraint => constraint.agentNum == problem.m_vAgents[conflictingIndex].agent.agentNum);
                    if (myMustConstraints.Count<P_Robust_Greedy_CbsConstraint>() != 0)
                    {
                        int maxMustConstraintTimeStep = myMustConstraints.Max<P_Robust_Greedy_CbsConstraint>(constraint => constraint.time);
        ...
        //    P_Robust_Greedy_AgentToCheckForCardinalConflicts[] agentsData = new P_Robust_Greedy_AgentToCheckForCardinalConflicts[this.problem.m_vAgents.Length];
        //    int[] groupSizes = this.GetGroupSizes();
        //    for (int i = 0; i < agentsData.Length; i++)
        //    {
        //        agentsData[i] = new P_Robust_Greedy_AgentToCheckForCardinalConflicts(
        //            /*this.mdds[i] != null,
        ...
        //    while (toCheck.Count != 0)
        //    {
        //        int i = ((P_Robust_Greedy_AgentToCheckForCardinalConflicts) toCheck.Remove()).index;
        ...
        //            this.conflictTimesPerAgent[i].Keys.Select<int, P_Robust_Greedy_AgentToCheckForCardinalConflicts>(
        //                num => agentsData[this.agentNumToIndex[num]]
        //            ).ToList<P_Robust_Greedy_AgentToCheckForCardinalConflicts>());

        //        while (otherAgents.Count != 0)
        //        {
        //            int otherAgentIndex = ((P_Robust_Greedy_AgentToCheckForCardinalConflicts) otherAgents.Remove()).index;
        ...
        public void AdoptSolutionOf(P_Robust_Greedy_CbsNode child)
        {
            Debug.Assert(this.totalCost == child.totalCost, "Tried to adopt node of a different cost");
            this.agentAExpansion = P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED;
            this.agentBExpansion = P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED;
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                HashSet<P_Robust_Greedy_CbsConstraint> constraints = this.GetConstraints();
        ...
        {
            P_Robust_Greedy_CbsNode other = (P_Robust_Greedy_CbsNode)obj;

            if (this.agentsGroupAssignment.SequenceEqual<ushort>(other.agentsGroupAssignment) == false)
                return false;

            P_Robust_Greedy_CbsNode current = this;
            HashSet<P_Robust_Greedy_CbsConstraint> other_constraints = other.GetConstraints();
            HashSet<P_Robust_Greedy_CbsConstraint> constraints = this.GetConstraints();
        ...
        {
            P_Robust_Greedy_CbsNode other = (P_Robust_Greedy_CbsNode)item;
        ...
        public int CompareToPRobust(P_Robust_Greedy_CbsNode other)  //for p-robust
        {
        ...
        public int CompareToIgnoreH(P_Robust_Greedy_CbsNode other, bool ignorePartialExpansion = false)
        {
            // Tie breaking:
            ...
            if (ignorePartialExpansion == false)
            {
                ...
                if ((this.agentAExpansion == P_Robust_Greedy_CbsNode.ExpansionState.DEFERRED || this.agentBExpansion == P_Robust_Greedy_CbsNode.ExpansionState.DEFERRED) &&
                    other.agentAExpansion == P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED && other.agentBExpansion == P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED)
                    return -1;
                if ((other.agentAExpansion == P_Robust_Greedy_CbsNode.ExpansionState.DEFERRED || other.agentBExpansion == P_Robust_Greedy_CbsNode.ExpansionState.DEFERRED) &&
                    this.agentAExpansion == P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED && this.agentBExpansion == P_Robust_Greedy_CbsNode.ExpansionState.NOT_EXPANDED)
                    return 1;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public P_Robust_Greedy_CbsConstraint GetLastConstraint()
        {
            return this.constraint;
        }

        public HashSet<P_Robust_Greedy_CbsConstraint> GetConstraints()
        {
            var constraints = new HashSet<P_Robust_Greedy_CbsConstraint>();
            int currentRange = 0;
            P_Robust_Greedy_CbsNode current = this;
                ...
                if (current.constraint != null && // Next check not enough if "surprise merges" happen (merges taken from adopted child)
                    ...
                    for (int i = -currentRange; i <= currentRange; i++)
                    {
                        ...
                        if (cbs.constraintPolicy == Run.ConstraintPolicy.DoubleRange && isLeftNode(current) && i != 0)
                            continue;
                        P_Robust_Greedy_CbsConstraint currentConstraint = current.constraint;
                        TimedMove     currentMove       = current.constraint.move;
                        P_Robust_Greedy_CbsConstraint newConstraint = new P_Robust_Greedy_CbsConstraint(currentConstraint.agentNum, currentMove.x, currentMove.y, currentMove.direction, currentMove.time + i);
        ...
        private bool isLeftNode(P_Robust_Greedy_CbsNode node)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<P_Robust_Greedy_CbsConstraint> GetConstraintsOrdered()
        {
            var constraints = new List<P_Robust_Greedy_CbsConstraint>();
            P_Robust_Greedy_CbsNode current = this;
            while (current.depth > 0) // The root has no constraints
            {
                if (current.constraint != null && // Next check not enough if "surprise merges" happen (merges taken from adopted child)
        ...
        public HashSet<P_Robust_Greedy_CbsConstraint> GetMustConstraints()
        {
            var constraints = new HashSet<P_Robust_Greedy_CbsConstraint>();
            P_Robust_Greedy_CbsNode current = this;
        ...
        public bool ShouldMerge(int mergeThreshold)
        {
           ...
           //     return false;
            P_Robust_Greedy_CbsNode current = this.prev;
            int a, b;
            while (current != null)
            {
        ...
        public bool DoesMustConstraintAllow(P_Robust_Greedy_CbsConstraint check)
        {
            P_Robust_Greedy_CbsNode current = this;
            while (current != null)
            {
        ...
        public void SetMustConstraint(P_Robust_Greedy_CbsConstraint set)
        {
        ...
        public bool Replan3b(int agentForReplan, int depthToReplan, int minCost = -1)
        {
            ...
            var CAT = (Dictionary_U<TimedMove, int>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CAT];
            var constraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];
            var mustConstraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problem.parameters[P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS];
            HashSet<P_Robust_Greedy_CbsConstraint> newMustConstraints = this.GetMustConstraints();

            if (newConstraints.Count != 0)
            {
                int maxConstraintTimeStep = newConstraints.Max<P_Robust_Greedy_CbsConstraint>(constraint => constraint.time);
                depthToReplan = Math.Max(depthToReplan, maxConstraintTimeStep); // Give all constraints a chance to affect the plan
            }
    ...
    /// </summary>
    public class P_Robust_Greedy_AgentToCheckForCardinalConflicts : IBinaryHeapItem
    {
        //public bool hasMDD;
        ...
        public P_Robust_Greedy_AgentToCheckForCardinalConflicts(/*bool hasMDD, int conflictingAgentsWithMDD,*/ int groupSize, int degree,
        ...
        public int CompareTo(IBinaryHeapItem item)
        {
            P_Robust_Greedy_AgentToCheckForCardinalConflicts other = (P_Robust_Greedy_AgentToCheckForCardinalConflicts)item;
