...
namespace CPF_experiment
{
    class CBS_IDA : ISolver
    {
        ...
        public virtual bool Expand(P_Robust_Optimal_CbsNode node, CbsConflict conflict)
        {
            if (runner.ElapsedMilliseconds() > Constants.MAX_TIME)
                return false;
            highLevelExpanded++;
            ...
            P_Robust_Optimal_CbsNode toAdd;
            P_Robust_Optimal_CbsConstraint con2 = new P_Robust_Optimal_CbsConstraint(conflict, instance, false);
            P_Robust_Optimal_CbsConstraint con1 = new P_Robust_Optimal_CbsConstraint(conflict, instance, true);
            byte stepLength=0;
            if (conflict.vertex)
                stepLength = 1;
            ...
            if (node.totalCost + conflict.timeStep + stepLength - node.PathLength(conflict.agentAIndex) <= fBound)
            {
                ...
                {
                    toAdd = new P_Robust_Optimal_CbsNode(node, con1, conflict.agentAIndex);
            ...
            if (node.totalCost + conflict.timeStep + stepLength - node.PathLength(conflict.agentBIndex) <= fBound)
            {
                ...
                {
                    toAdd = new P_Robust_Optimal_CbsNode(node, con2, conflict.agentBIndex);
            ...
            if (ok1 && ok2)
            {
                ...
                if (toAdd.Replan3b(conflict.agentAIndex, Math.Max(minCost, conflict.timeStep)))
                {
                    if (toAdd.totalCost <= fBound)
                    {
                        toAdd = new P_Robust_Optimal_CbsNode(toAdd, con2, conflict.agentBIndex);
        ...
        public void Setup(ProblemInstance problemInstance, Run runner)
        {
            ...
            if (problemInstance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.CAT) == false) // Top-most CBS only
            {
                problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CAT] = new HashSet_U<TimedMove>();
                problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS] = new HashSet_U<P_Robust_Optimal_CbsConstraint>();
                problemInstance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS] = new List<P_Robust_Optimal_CbsConstraint>();
                this.topMost = true;
            }
            else
                this.topMost = false;
        ...
        protected bool MergeConflicting(P_Robust_Optimal_CbsNode node)
        {
