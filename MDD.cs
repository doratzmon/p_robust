...
namespace CPF_experiment
{
    ...
    public class MDD
    {
        ...
        public MDD(int mddNum, int agentNum, Move start_pos, int cost, int numOfLevels, int numOfAgents, ProblemInstance instance, bool ignoreConstraints = false, bool supportPruning = true)
        {
            ...
            if (ignoreConstraints == false && instance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS) &&
                    ((HashSet_U<P_Robust_Optimal_CbsConstraint>)instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS]).Count != 0)
            {
                this.queryConstraint = new P_Robust_Optimal_CbsConstraint();
                this.queryConstraint.queryInstance = true;

                this.constraints = (HashSet_U<P_Robust_Optimal_CbsConstraint>)instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.CONSTRAINTS];
            }

            if (ignoreConstraints == false && instance.parameters.ContainsKey(P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS) &&
                 ((HashSet_U<P_Robust_Optimal_CbsConstraint>)instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS]).Count != 0)
            {
                // TODO: Code dup with ClassicAStar's constructor
                var musts = (HashSet_U<P_Robust_Optimal_CbsConstraint>)instance.parameters[P_Robust_Optimal_CBS_LocalConflicts.MUST_CONSTRAINTS];
