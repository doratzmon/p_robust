...
{   
    ...
    /// </summary>
    public class P_Robust_Greedy_ClassicAStar : ICbsSolver, IMStarSolver
    {
        ...
        protected HashSet_U<P_Robust_Greedy_CbsConstraint> constraints;
        /// <summary>
        ...
        protected Dictionary<WorldState, HashSet<P_Robust_Greedy_CbsConstraint>[]> mstarPlanBasesToTheirConstraints; // Most nodes won't have constraints so I don't want to make their memory footprint needlessly large.
        ...
        /// Default constructor.
        /// </summary>
        public P_Robust_Greedy_ClassicAStar(HeuristicCalculator heuristic = null, bool mStar = false, bool mStarShuffle = false, int conflictRange = 0)
        {
            this.closedList = new Dictionary<WorldState, WorldState>();
            this.openList = new OpenList(this);
            this.heuristic = heuristic;
            
            this.queryConstraint = new P_Robust_Greedy_CbsConstraint();
        ...
        public virtual void Setup(ProblemInstance problemInstance, int minDepth, Run runner, int minCost)
        {
            ...
            if (problemInstance.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS) &&
                ((HashSet_U<P_Robust_Greedy_CbsConstraint>)problemInstance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS]).Count != 0)
                 this.constraints = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problemInstance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];
 
           /*  if (problemInstance.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS) &&
                 ((HashSet_U<P_Robust_Greedy_CbsConstraint>)problemInstance.parameters[P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS]).Count != 0)
             {
                 var musts = (HashSet_U<P_Robust_Greedy_CbsConstraint>)problemInstance.parameters[P_Robust_Greedy_CBS_LocalConflicts.MUST_CONSTRAINTS];
                 this.mustConstraints = new Dictionary<int, TimedMove>[musts.Max<P_Robust_Greedy_CbsConstraint>(con => con.GetTimeStep()) + 1]; // To have index MAX, array needs MAX + 1 places.
                 foreach (P_Robust_Greedy_CbsConstraint con in musts)
                 {
        ...
        public virtual bool Solve()
        {
            ...
            while (openList.Count > 0)
            {
                ...
                {
                    //((IReadOnlyDictionary<TimedMove, List<int>>)instance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CAT]), conflictRange);
        ...
        /// </summary>
        private P_Robust_Greedy_CbsConstraint queryConstraint;
        ...
        protected virtual bool ProcessGeneratedNode(WorldState currentNode)
        {
            ...
            // within the budget
            {
                ...
                if (instance.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.CAT))
                {
                    // Accumulating the conflicts count from parent to child.
                    // We're counting conflicts along the entire path, so the parent's conflicts count is added to the child's:
                    currentNode.cbsInternalConflicts = new Dictionary<int,int>(currentNode.prevStep.cbsInternalConflicts);
        ...
        //    if (this.instance.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS))
        //        constraints = (HashSet_U<CbsConstraint>)this.instance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS];
        //    else
        //    {
        //        constraints = new HashSet_U<CbsConstraint>();
        //        this.instance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CONSTRAINTS] = constraints;
        //    }

        //    if (this.instance.parameters.ContainsKey(P_Robust_Greedy_CBS_LocalConflicts.CAT) == false)
        //        this.instance.parameters[P_Robust_Greedy_CBS_LocalConflicts.CAT] = new Dictionary_U<TimedMove, int>(); // Indicate TO CBS that another level is running above it
        ...
        //    ICbsSolver solver = new P_Robust_Greedy_CBS_LocalConflicts(astar, astar); // Uses a precomputed solution if possible
